...
This is a small tool, written in Go, for working with `spn-core` history archives directly.
It is a standalone tool that does not require `spn-core`, or any other programs.

It is much smaller and simpler than `spn-core`, and is intended only for archive-maintenance tasks.
...
```
$ go install github.com/spn/go/tools/spn-archivist
```

## Usage

```
inspect spn history archive

Usage:
  spn-archivist [flags]
  spn-archivist [command]
...
Use "spn-archivist [command] --help" for more information about a command.
```

## Specifying history archives

Unlike `spn-core`, `spn-archivist` does not run subprocesses to access history archives;
...
`spn-archivist` supports reading from and writing to any S3-compatible storage.
...
```
$ spn-archivist status --s3endpoint ams3.digitaloceanspaces.com s3://bucketname/prefix
```
...
```
$ spn-archivist status http://s3-eu-west-1.amazonaws.com/history.spn.org/prd/core-testnet/core_testnet_001

       Archive: http://s3-eu-west-1.amazonaws.com/history.spn.org/prd/core-testnet/core_testnet_001
...
```
$ spn-archivist mirror http://s3-eu-west-1.amazonaws.com/history.spn.org/prd/core-testnet/core_testnet_001 file://local-archive

2016/02/10 18:27:09 mirroring http://s3-eu-west-1.amazonaws.com/history.spn.org/prd/core-testnet/core_testnet_001 -> file://local-archive
...
```
$ spn-archivist --last 1024 mirror http://s3-eu-west-1.amazonaws.com/history.spn.org/prd/core-testnet/core_testnet_001 file://local-archive

2016/02/10 19:14:01 mirroring http://s3-eu-west-1.amazonaws.com/history.spn.org/prd/core-testnet/core_testnet_001 -> file://local-archive
...
```
$ spn-archivist scan file://local-archive
...
```
$ spn-archivist --last 4096 scan file://local-archive
...
$ spn-archivist --low 1000000 --high 1006000 scan file://local-archive
...
```
$ spn-archivist --verify --last 4096 scan file://local-archive
...
$ spn-archivist repair file://local-archive file://broken-archive
...
```
 spn-archivist dumpxdr local-archive/transactions//00/20/de/transactions-0020de7f.xdr.gz
