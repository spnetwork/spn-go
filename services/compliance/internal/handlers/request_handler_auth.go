...
import (
	...
	baseAmount "github.com/spn/go/amount"
	"github.com/spn/go/protocols/compliance"
	"github.com/spn/go/services/compliance/internal/db"
	shared "github.com/spn/go/services/internal/bridge-compliance-shared"
	httpHelpers "github.com/spn/go/services/internal/bridge-compliance-shared/http/helpers"
	callback "github.com/spn/go/services/internal/bridge-compliance-shared/protocols/compliance"
...
func (rh *RequestHandler) HandlerAuth(w http.ResponseWriter, r *http.Request) {
	...
	senderSpnToml, err := rh.SpnTomlResolver.GetSpnTomlByAddress(authData.Sender)
	if err != nil {
		log.WithFields(log.Fields{"err": err, "sender": authData.Sender}).Warn("Cannot get spn.toml of sender")
		errorResponse := httpHelpers.NewInvalidParameterError("data.sender", "Cannot get spn.toml of sender")
		httpHelpers.Write(w, errorResponse)
		return
	}

	if !shared.IsValidAccountID(senderSpnToml.SigningKey) {
		errorResponse := httpHelpers.NewInvalidParameterError("data.sender", "SIGNING_KEY in spn.toml of sender is invalid")
		// TODO
		// log.WithFields(errorResponse.LogData).Warn("SIGNING_KEY in spn.toml of sender is invalid")
		httpHelpers.Write(w, errorResponse)
		return
	}
	...
	err = rh.SignatureSignerVerifier.Verify(senderSpnToml.SigningKey, []byte(authreq.DataJSON), signatureBytes)
	if err != nil {
		log.WithFields(log.Fields{
			"signing_key": senderSpnToml.SigningKey,
			"data":        authreq.Data,
