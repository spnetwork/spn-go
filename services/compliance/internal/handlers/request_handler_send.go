...
import (
	...
	"github.com/spn/go/protocols/federation"
...
func (rh *RequestHandler) HandlerSend(w http.ResponseWriter, r *http.Request) {
	...
	if authDataEntity != nil {
		spnToml, err := rh.SpnTomlResolver.GetSpnToml(authDataEntity.Domain)
		if err != nil {
		...
		if spnToml.AuthServer == "" {
			log.Print("No AUTH_SERVER in spn.toml")
			helpers.Write(w, callback.AuthServerNotDefined)
			return
		}

		rh.sendAuthData(w, spnToml.AuthServer, []byte(authDataEntity.AuthData))
		return
	}
	...
	spnToml, err := rh.SpnTomlResolver.GetSpnToml(domain)
	if err != nil {
		log.WithFields(log.Fields{
			"destination": request.Destination,
			"err":         err,
	...
	rh.sendAuthData(w, spnToml.AuthServer, data)
}
