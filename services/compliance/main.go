...
import (
	...
	"github.com/spn/go/services/compliance/internal/handlers"
	supportConfig "github.com/spn/go/support/config"
	"github.com/spn/go/support/db/schema"
	"github.com/spn/go/support/errors"
	supportHttp "github.com/spn/go/support/http"
)
...
func init() {
	rootCmd = &cobra.Command{
		Use:   "compliance",
		Short: "spn compliance server",
		Long:  `spn compliance server`,
		Run:   run,
	}
...
func NewApp(config config.Config, migrateFlag bool, versionFlag bool, version string) (app *App, err error) {
	...
	err = g.Provide(
		...
		&inject.Object{Value: &spntomlClient},
