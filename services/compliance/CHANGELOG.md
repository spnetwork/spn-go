...
* Fix for a bug in `/builder` endpoint: sequence number is now incremented when loaded from Horizon server (https://github.com/spn/bridge-server/issues/86).
...
* Added error messages to Compliance protocol ([SEP-0003](https://github.com/spn/spn-protocol/blob/master/ecosystem/sep-0003.md))

## 0.0.27

* Admin Panel (`/admin` endpoint in `bridge` server).
* `/tx_status` endpoint [More info](https://github.com/spn/spn-protocol/blob/master/ecosystem/sep-0001.md).
...
* [XLM (lumen)](https://www.spn.org/lumens/) payments can be now used in `PaymentListener`.
...
* Update `github.com/spn/go` dependency.
...
* Change spn.toml location to new standard.
...
* [Compliance protocol](https://www.spn.org/developers/learn/integration-guides/compliance-protocol.html) support.
...
* Sending to Spn address with memo in `/send`.
