...
func (ac *AccountConfigurator) Start() error {
	ac.log = common.CreateLogger("SpnAccountConfigurator")
	ac.log.Info("SpnAccountConfigurator starting")
...
func (ac *AccountConfigurator) ConfigureAccount(destination, assetCode, amount string) {
	...
	localLog.Info("Configuring Spn account")
	...
	for {
		...
		localLog.WithField("destination", destination).Info("Creating Spn account")
		err = ac.createAccountTransaction(destination)
		if err != nil {
			localLog.WithField("err", err).Error("Error creating Spn account")
			time.Sleep(2 * time.Second)
			continue
		}
