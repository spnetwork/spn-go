...
import (
	...
	"github.com/spn/go/services/bifrost/server"
	"github.com/spn/go/services/bifrost/sse"
	"github.com/spn/go/services/bifrost/spn"
	"github.com/spn/go/services/bifrost/stress"
	supportConfig "github.com/spn/go/support/config"
...
var rootCmd = &cobra.Command{
	Use:   "bifrost",
	Short: "Bridge server to allow participating in Spn based ICOs using Bitcoin and Ethereum",
}
...
This command will create 3 server.Server's listening on ports 8000-8002.`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		for i := 0; i < numServers; i++ {
			go func(i int) {
				cfg.Port = ports[i]
				cfg.Spn.SignerSecretKey = signers[i]
		...
		users := stress.Users{
			...
			NetworkPassphrase: cfg.Spn.NetworkPassphrase,
			UsersPerSecond:    usersPerSecond,
			BifrostPorts:      ports,
			IssuerPublicKey:   cfg.Spn.IssuerPublicKey,
		}
...
func init() {
	// TODO I think these should be default in spn/go:
...
func createServer(cfg config.Config, stressTest bool) *server.Server {
	...
	spnAccountConfigurator := &spn.AccountConfigurator{
		NetworkPassphrase:     cfg.Spn.NetworkPassphrase,
		IssuerPublicKey:       cfg.Spn.IssuerPublicKey,
		DistributionPublicKey: cfg.Spn.DistributionPublicKey,
		SignerSecretKey:       cfg.Spn.SignerSecretKey,
		NeedsAuthorize:        cfg.Spn.NeedsAuthorize,
		TokenAssetCode:        cfg.Spn.TokenAssetCode,
		StartingBalance:       cfg.Spn.StartingBalance,
		LockUnixTimestamp:     cfg.Spn.LockUnixTimestamp,
	}

	if cfg.Spn.StartingBalance == "" {
		spnAccountConfigurator.StartingBalance = "2.1"
	}

	if cfg.Bitcoin != nil {
		spnAccountConfigurator.TokenPriceBTC = cfg.Bitcoin.TokenPrice
	}

	if cfg.Ethereum != nil {
		spnAccountConfigurator.TokenPriceETH = cfg.Ethereum.TokenPrice
	}
	...
	err = g.Provide(
		...
		&inject.Object{Value: spnAccountConfigurator},
	)
	if err != nil {
