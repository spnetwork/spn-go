...
func (s *Server) onSpnAccountCreated(destination string) {
	association, err := s.Database.GetAssociationBySpnPublicKey(destination)
	if err != nil {
	...
	if association == nil {
		s.log.WithField("spnPublicKey", destination).Error("Association not found")
		return
	}
