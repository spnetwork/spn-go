...
func (s *Server) Start() error {
	...
	s.SpnAccountConfigurator.OnAccountCreated = s.onSpnAccountCreated
	s.SpnAccountConfigurator.OnExchanged = s.onExchanged
	s.SpnAccountConfigurator.OnExchangedTimelocked = s.OnExchangedTimelocked
	...
	err := s.SpnAccountConfigurator.Start()
	if err != nil {
		return errors.Wrap(err, "Error starting SpnAccountConfigurator")
	}
...
func (s *Server) handlerGenerateAddress(w stdhttp.ResponseWriter, r *stdhttp.Request, chain database.Chain) {
	...
	spnPublicKey := r.PostFormValue("spn_public_key")
	_, err := keypair.Parse(spnPublicKey)
	if err != nil || (err == nil && spnPublicKey[0] != 'G') {
		log.WithField("spnPublicKey", spnPublicKey).Warn("Invalid spnPublicKey")
		w.WriteHeader(stdhttp.StatusBadRequest)
		return
	}
	...
	err = s.Database.CreateAddressAssociation(chain, spnPublicKey, address, index)
	if err != nil {
		log.WithFields(log.F{
			"err":              err,
			"chain":            chain,
			"index":            index,
			"spnPublicKey": spnPublicKey,
			"address":          address,
