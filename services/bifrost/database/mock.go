...
func (m *MockDatabase) CreateAddressAssociation(chain Chain, spnAddress, address string, addressIndex uint32) error {
	a := m.Called(chain, spnAddress, address, addressIndex)
	return a.Error(0)
}
...
func (m *MockDatabase) GetAssociationBySpnPublicKey(spnPublicKey string) (*AddressAssociation, error) {
	a := m.Called(spnPublicKey)
	return a.Get(0).(*AddressAssociation), a.Error(1)
}
