...
func (d *PostgresDatabase) CreateAddressAssociation(chain Chain, spnAddress, address string, addressIndex uint32) error {
...
func (d *PostgresDatabase) GetAssociationBySpnPublicKey(spnPublicKey string) (*AddressAssociation, error) {
	addressAssociationTable := d.getTable(addressAssociationTableName, nil)
	row := &AddressAssociation{}
	where := map[string]interface{}{"spn_public_key": spnPublicKey}
	err := addressAssociationTable.Get(row, where).Exec()
	if err != nil {
		switch errors.Cause(err) {
		case sql.ErrNoRows:
