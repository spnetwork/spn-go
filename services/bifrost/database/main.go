...
type Database interface {
	// CreateAddressAssociation creates Bitcoin/Ethereum-Spn association. `addressIndex`
	// is the chain (Bitcoin/Ethereum) address derivation index (BIP-32).
	CreateAddressAssociation(chain Chain, spnAddress, address string, addressIndex uint32) error
	// GetAssociationByChainAddress searches for previously saved Bitcoin/Ethereum-Spn association.
	// Should return nil if not found.
	GetAssociationByChainAddress(chain Chain, address string) (*AddressAssociation, error)
	// GetAssociationBySpnPublicKey searches for previously saved Bitcoin/Ethereum-Spn association.
	// Should return nil if not found.
	GetAssociationBySpnPublicKey(spnPublicKey string) (*AddressAssociation, error)
...
type AddressAssociation struct {
	...
	SpnPublicKey string    `db:"spn_public_key"`
