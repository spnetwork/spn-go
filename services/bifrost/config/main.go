...
type Config struct {
	...
	Spn struct {
		...
		IssuerPublicKey string `valid:"required,spn_accountid" toml:"issuer_public_key"`
		...
		DistributionPublicKey string `valid:"required,spn_accountid" toml:"distribution_public_key"`
		// SignerSecretKey is:
		...
		// https://www.spn.org/developers/guides/channels.html
		// Signer's sequence number will be consumed in transaction's sequence number.
		SignerSecretKey string `valid:"required,spn_seed" toml:"signer_secret_key"`
		...
		StartingBalance string `valid:"optional,spn_amount" toml:"starting_balance"`
...
func (c Config) SignerPublicKey() string {
	kp := keypair.MustParse(c.Spn.SignerSecretKey)
	return kp.Address()
}
