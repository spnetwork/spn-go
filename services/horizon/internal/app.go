...
import (
	...
	horizonContext "github.com/spn/go/services/horizon/internal/context"
	...
	"github.com/spn/go/services/horizon/internal/operationfeestats"
	"github.com/spn/go/services/horizon/internal/paths"
	"github.com/spn/go/services/horizon/internal/reap"
...
// CoreSession returns a new session that loads data from the spn core
// database. The returned session is bound to `ctx`.
...
// UpdateSpnCoreInfo updates the value of coreVersion and networkPassphrase
// from the Spn core API.
func (a *App) UpdateSpnCoreInfo() {
	if a.config.SpnCoreURL == "" {
		return
	}

	fail := func(err error) {
		log.Warnf("could not load spn-core info: %s", err)
	}

	core := &spncore.Client{
		URL: a.config.SpnCoreURL,
	}
...
func (a *App) Tick() {
	...
	// update ledger state, operation fee state, and spn-core info in parallel
	wg.Add(3)
	go func() { a.UpdateLedgerState(); wg.Done() }()
	go func() { a.UpdateOperationFeeStatsState(); wg.Done() }()
	go func() { a.UpdateSpnCoreInfo(); wg.Done() }()
	wg.Wait()
