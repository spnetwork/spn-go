...
import (
	...
	"github.com/spn/go/meta"
	"github.com/spn/go/services/horizon/internal/db2/history"
	"github.com/spn/go/services/horizon/internal/ingest/participants"
	"github.com/spn/go/support/errors"
	ilog "github.com/spn/go/support/log"
	sTime "github.com/spn/go/support/time"
...
func (is *Session) ingestEffects() {
	...
	case xdr.OperationTypeCreatePassiveOffer:
		...
		// KNOWN ISSUE:  spn-core creates results for CreatePassiveOffer operations
...
func (is *Session) ingestTrades() {
	...
	for i, trade := range trades {
		// spn-core will opportunisticly garbage collect invalid offers (in the
...
// reportCursorState makes an http request to the configured spn-core server
// to report that it has finished processing the data being ingested.  This
// allows spn-core to free that storage when next it runs its own
// maintenance.
func (is *Session) reportCursorState() error {
	...
	core := &spncore.Client{URL: is.SpnCoreURL}
