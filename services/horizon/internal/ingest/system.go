...
import (
	...
	herr "github.com/spn/go/services/horizon/internal/errors"
	"github.com/spn/go/services/horizon/internal/ledger"
	"github.com/spn/go/services/horizon/internal/toid"
	"github.com/spn/go/support/errors"
	ilog "github.com/spn/go/support/log"
)
...
// RebaseHistory re-establishes horizon's history database by clearing it, ingesting the latest ledger in spn-core then backfilling as many ledgers as possible
...
// run causes the importer to check spn-core to see if we can import new
// data.
func (i *System) runOnce() {
	...
	if coreLatest == 1 {
		log.Warn("ingest: waiting for spn-core sync")
		return
	}
...
// horizon but is no longer present in the spn-core database source.  The
// usual cause for this situation is a spn-core that uses the CATCHUP_RECENT
// mode.
