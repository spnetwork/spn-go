...
clientData:
  laboratoryUrl: https://www.spn.org/laboratory/#explorer?resource=operations&endpoint=all
---
...
This endpoint can also be used in [streaming](../streaming.md) mode so it is possible to use it to listen as operations are processed in the Spn network.
...
```sh
curl "https://horizon-testnet.spn.org/operations?limit=200&order=desc"
```
