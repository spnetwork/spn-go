...
using JavaScript and `EventSource`.  We will eschew using [`js-spn-sdk`](https://github.com/spn/js-spn-sdk), the
...
```javascript
var Keypair = require("spn-base").Keypair;
...
The Spn test network provides the Friendbot, a tool that developers
can use to get testnet lumens for testing purposes. To fund your account, simply
execute the following curl command:

```bash
$ curl "https://friendbot.spn.org/?addr=GB7JFK56QXQ4DVJRNPDBXABNG3IVKIXWWJJRJICHRU22Z5R5PI65GAK3"
```
...
After a few seconds, the Spn network will perform consensus, close the
...
```bash
$ curl -H "Accept: text/event-stream" "https://horizon-testnet.spn.org/accounts/GB7JFK56QXQ4DVJRNPDBXABNG3IVKIXWWJJRJICHRU22Z5R5PI65GAK3/payments"
```
...
> If you need a reliable streaming connection please use our [SDK](https://github.com/spn/js-spn-sdk).
...
var es = new EventSource('https://horizon-testnet.spn.org/accounts/GB7JFK56QXQ4DVJRNPDBXABNG3IVKIXWWJJRJICHRU22Z5R5PI65GAK3/payments');
...
```bash
$ curl "https://horizon-testnet.spn.org/accounts/GB7JFK56QXQ4DVJRNPDBXABNG3IVKIXWWJJRJICHRU22Z5R5PI65GAK3"
```
...
```js
var SpnBase = require("spn-base");
SpnBase.Network.useTestNetwork();

var keypair = SpnBase.Keypair.fromSecret('SCU36VV2OYTUMDSSU4EIVX4UUHY3XC7N44VL4IJ26IOG6HVNC7DY5UJO');
var account = new SpnBase.Account(keypair.publicKey(), "713226564141056");

var amount = "100";
var transaction = new SpnBase.TransactionBuilder(account)
  .addOperation(SpnBase.Operation.createAccount({
    destination: SpnBase.Keypair.random().publicKey(),
