...
import (
	...
	"github.com/spn/go/services/horizon/internal/logmetrics"
	tdb "github.com/spn/go/services/horizon/internal/test/db"
...
// LoadScenarioWithoutHorizon populates the test Spn core database a with
...
func Start(t *testing.T) *T {
	...
	result.CoreDB = SpnCoreDatabase(t)
...
// SpnCoreDatabase returns a connection to the spn core test database
//
// DEPRECATED:  use `SpnCore()` from test/db package
func SpnCoreDatabase(t *testing.T) *sqlx.DB {
	return tdb.SpnCore(t)
}

// SpnCoreDatabaseURL returns the database connection the url any test
// use when connecting to the spn-core database
//
// DEPRECATED:  use `SpnCoreURL()` from test/db package
func SpnCoreDatabaseURL() string {
	return tdb.SpnCoreURL()
}
