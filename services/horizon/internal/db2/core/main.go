...
// reading rows from a Spn Core db
package core
...
// Offer is row of data from the `offers` table from spn-core
type Offer struct {
...
// Q is a helper struct on which to hang common queries against a spn
// core database.
type Q struct {
	*db.Session
}
...
// Signer is a row of data from the `signers` table from spn-core
type Signer struct {
	Accountid string
	Publickey string
	Weight    int32
...
// Transaction is row of data from the `txhistory` table from spn-core
...
// TransactionFee is row of data from the `txfeehistory` table from spn-core
...
// Trustline is a row of data from the `trustlines` table from spn-core
...
// spn-core database this ingestion system is communicating with.  Horizon,
