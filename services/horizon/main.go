...
import (
	...
	"github.com/spn/go/services/horizon/internal"
...
func init() {
	...
	viper.BindEnv("spn-core-db-url", "STELLAR_CORE_DATABASE_URL")
	viper.BindEnv("spn-core-url", "STELLAR_CORE_URL")
	...
	rootCmd = &cobra.Command{
		Use:   "horizon",
		Short: "client-facing api server for the spn network",
		Long:  "client-facing api server for the spn network",
	...
	rootCmd.PersistentFlags().String(
		"spn-core-db-url",
		"",
		"spn-core postgres database to connect with",
	)

	rootCmd.PersistentFlags().String(
		"spn-core-url",
		"",
		"spn-core to connect with (for http commands)",
	)
	...
	rootCmd.PersistentFlags().Bool(
		"ingest",
		false,
		"causes this horizon process to ingest data from spn-core into horizon's db",
	)
	...
	rootCmd.PersistentFlags().Uint(
		...
		0,
		"the maximum number of ledgers the history db is allowed to be out of date from the connected spn-core db before horizon considers history stale",
	)
...
func initConfig() {
	...
	if viper.GetString("spn-core-db-url") == "" {
		stdLog.Fatal("Invalid config: spn-core-db-url is blank.  Please specify --spn-core-db-url on the command line or set the STELLAR_CORE_DATABASE_URL environment variable.")
	}

	if viper.GetString("spn-core-url") == "" {
		stdLog.Fatal("Invalid config: spn-core-url is blank.  Please specify --spn-core-url on the command line or set the STELLAR_CORE_URL environment variable.")
	}
	...
	config = horizon.Config{
		...
		SpnCoreDatabaseURL: viper.GetString("spn-core-db-url"),
		SpnCoreURL:         viper.GetString("spn-core-url"),
