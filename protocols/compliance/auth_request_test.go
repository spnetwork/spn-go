...
func TestValidateError(t *testing.T) {
	...
	authData := &AuthData{
		Sender:         "bartekspn.org",
		NeedInfo:       false,
		Tx:             "&^%",
		AttachmentJSON: "abc",
	}

	assert.EqualError(t, authData.Validate(), "sender: bartekspn.org does not validate as spn_address;tx: &^% does not validate as base64;attachment: abc does not validate as json")
}
