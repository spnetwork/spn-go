...
// to sender server in order to obtain spn.toml file and signing key.
func (r *AuthRequest) VerifySignature(sender string) error {
	...
	senderSpnToml, err := spntoml.GetSpnTomlByAddress(sender)
	if err != nil {
		return errors.Wrap(err, "Cannot get spn.toml of sender domain")
	}

	if senderSpnToml.SigningKey == "" {
		return errors.New("No SIGNING_KEY in spn.toml of sender")
	}

	kp, err := keypair.Parse(senderSpnToml.SigningKey)
	if err != nil {
