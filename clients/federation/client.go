...
// LookupByAddress performs a federated lookup following to the spn
...
// "name" type is a legacy holdover from the legacy spn network's federation
...
// LookupByAccountID performs a federated lookup following to the spn
...
// ForwardRequest performs a federated lookup following to the spn
...
func (c *Client) getFederationServer(domain string) (string, error) {
	stoml, err := c.SpnTOML.GetSpnToml(domain)
	if err != nil {
		return "", errors.Wrap(err, "get spn.toml failed")
	}

	if stoml.FederationServer == "" {
		return "", errors.New("spn.toml is missing federation server info")
	}
