...
// spn-core server using HTTP
type Client struct {
	// HTTP is the client to use when communicating with spn-core.  If nil,
	// http.DefaultClient will be used.
	HTTP HTTP

	// URL of Spn Core server to connect.
	URL string
}

// Info calls the `info` command on the connected spn core and returns the
// provided response
...
// SetCursor calls the `setcursor` command on the connected spn core
func (c *Client) SetCursor(ctx context.Context, id string, cursor int32) error {
	...
	if body != SetCursorDone {
		return errors.Errorf("failed to set cursor on spn-core: %s", body)
	}

	return nil
}

// SubmitTransaction calls the `tx` command on the connected spn core with the provided envelope
...
// WaitForNetworkSync continually polls the connected spn-core until it
...
// simpleGet returns a new GET request to the connected spn-core using the
